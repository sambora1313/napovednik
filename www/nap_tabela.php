...
// ime dneva izbranega dneva
$dan = $_POST["dan"];
$dan = trim($dan);

// izbrani datum v formatu yy_mm_dd
$datum = $_POST["datum"];
$datum = trim($datum);
...
// pričakovano ime iskane tabele
$tabela = $datum . '__' . $termin;

echo $tabela;

$connect = mysqli_connect("database", "root", $_ENV['MYSQL_ROOT_PASSWORD'], "napovednik_test");

$check = $connect->query("SHOW TABLES LIKE '" . $tabela . "'");
...
if ($check->num_rows == 1) {
    ...
    $query = "SELECT * FROM $tabela ORDER BY ura ASC";
    ...
    echo "Tabela ne obstaja";
    return FALSE;
}

//$query = "SELECT * FROM ponedeljek_dop ORDER BY ura ASC";
//$result = mysqli_query($connect, $query);
